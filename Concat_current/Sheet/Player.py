...
class Player:
    ...
    def upd_ASIC(self,sender,data):
        ...
        if data == "ASI":
            self.DATA.C.ASI.feat[place]=""
        elif data == "FEAT":
            self.DATA.C.ASI.asi[place]=["",""]
    ...
    def config_FEAT(self):

        for level, feat in self.SHEET.FEAT.items():
            if feat:
                data = self.SHEET.FEAT[level][next(iter(feat))].Data
                if "init" in data:
                    self.SHEET.INIT.feat = int(data.init)
                    
                    
                if "ATR" in data:
                    atr_data = self.SHEET.FEAT[level][next(iter(feat))].Data.ATR
                    if "choice" in atr_data:
                        pass
                    elif "stat" in atr_data:
                        pass
        self.config_INIT()



    def config_ASIV(self):
        self.clear_data("FEAT_data")
        for stat in atrL:
            self.ATR[stat].data[2]=sum(row.count(stat) for row in self.DATA.C.ASI.asi)
        
        for index, item in enumerate(self.DATA.C.ASI.feat):
            if item != "":
                self.SHEET.FEAT[f"L{index}"]={}
                self.SHEET.FEAT[f"L{index}"][item]=dfeat[item]
        
        self.config_FEAT()
        
        
        self.upd_ui(["ASI","ASIC"])
        self.config_ATR()
    # #-------------------------------------
    ...
    def config_INIT(self):
        self.SHEET.INIT.v=self.SHEET.INIT.feat+self.ATR.DEX.m
    ...
    def config_ASI(self):
        for index, i in enumerate(dclass[self.CORE.C].ASI):
            if self.CORE.L >= i:
                ...
                self.DATA.C.ASI.choice[index] = ""
                self.DATA.C.ASI.feat[index] = ""
                self.DATA.C.ASI.asi[index] = ["",""]
                self.SHEET.FEAT[f'L{index}']={}
        self.config_ASIV()
    ...
    def clear_data(self,sender):    
        if sender == "RACE_data":
            ...
            self.SHEET.FEATURE={}
        elif sender == "FEAT_data":
            self.SHEET.INIT.feat=0
            for stat in atrL:
                self.ATR[stat].data[3]=0
